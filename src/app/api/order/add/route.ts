...
/** Proxy for shop-api Order/AddOrder. Forwards the caller's bearer token. */
export async function POST(req: Request) {
  ...
  try {
    const upstream = await fetch(`${SHOP_BASE_URL}/Order/AddOrder`, {
      method: "POST",
      headers: {
        accept: "text/plain",
    ...
    return NextResponse.json(
      { success: false, errorMessage: "خطا در ثبت سفارش" },
      { status: 502 },
    );
  }
}
