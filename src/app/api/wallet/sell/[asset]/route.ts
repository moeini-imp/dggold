...
/** Proxy for wallet-api Payments/Sell/{asset}. Forwards the bearer token. */
export async function POST(
  ...
  try {
    const upstream = await fetch(`${WALLET_BASE_URL}/Payments/Sell/${asset}`, {
      method: "POST",
      headers: {
        accept: "text/plain",
    ...
    return NextResponse.json(
      { success: false, errorMessage: "خطا در ثبت فروش" },
      { status: 502 },
    );
  }
}
